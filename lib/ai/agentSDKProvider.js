...
function classifyAgentSdkError(err) {
  const message = String(err?.message || err || '').toLowerCase();

  if (message.includes('login') || message.includes('auth') || message.includes('unauthorized')) {
    return 'CLAUDE_SESSION_EXPIRED';
  }
  if (message.includes('quota') || message.includes('rate limit') || message.includes('rate_limit')) {
    return 'CLAUDE_QUOTA_EXHAUSTED';
  }
  if (message.includes('not found') || message.includes('enoent') || message.includes('command failed')) {
    return 'CLAUDE_SDK_UNAVAILABLE';
  }
  if (message.includes('aborted') || message.includes('abort')) {
    return 'ABORTED';
  }

  return 'SDK_QUERY_FAILED';
}

function classifyAgentSdkResult(result) {
  const message = String(result?.result || result?.subtype || '').toLowerCase();
  ...
  async generate({ systemPrompt, userPrompt, abortSignal, model }) {
    ...
    try {
      ...
      throw new ProviderError(`Agent SDK query failed: ${err.message}`, {
        cause: err,
        code: classifyAgentSdkError(err),
      });
    }
    ...
    if (result.subtype !== 'success' || result.is_error) {
      throw new ProviderError(
        ...
        { code: classifyAgentSdkResult(result) }
      );
    }
